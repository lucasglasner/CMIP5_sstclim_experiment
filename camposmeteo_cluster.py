...
paths_picontrol_pr   = sorted(glob("cmip5/piControl/pr/regrid/anomaly/*_yanomaly.nc"))   #Path to sstclim pr anomalies
paths_picontrol_ts   = sorted(glob("cmip5/piControl/ts/regrid/anomaly/*_yanomaly.nc"))   #Path to sstclim ts anomalies
paths_picontrol_psl  = sorted(glob("cmip5/piControl/psl/regrid/anomaly/*_yanomaly.nc"))  #Path to sstclim psl anomalies
exp = "picontrol"
season="annual"
#Create data bag
...
for n in range(len(models)):
    ...
    for var in ["pr","ts","psl"]:
        ...
        if "yanomaly" in paths:
            ...
            dic2 = xr.merge([dic.groupby("time.month")[i] for i in [4,5,6,7,8,9]])[var].squeeze()
...
for mod in models:
    dry_times[mod] = np.where(list(map(lambda x: float(x)<np.quantile(pr_qn[mod],0.1),pr_qn[mod].squeeze())))[0]
    wet_times[mod] = np.where(list(map(lambda x: float(x)>np.quantile(pr_qn[mod],0.9),pr_qn[mod].squeeze())))[0]
...
# kmeans   = KMeans(n_clusters=3,init="k-means++",n_init=99).fit(whiten(np.hstack([matrix[var] for var in ["pr","ts","psl"]])))
